...
if (isServer) then {
    [{
        ...
        if (GVAR(Game) getVariable QGVAR(game_state) == "ENDED") exitWith {
...
if (isServer) then {
    [_carrier] spawn {
        while { GVAR(Game) getVariable QGVAR(game_state) != "ENDED" } do {
            params ["_carrier"];
...
if (isServer) then {
    [_side] spawn {
        ...
        GVAR(Game) setVariable [QGVAR(game_state), "ENDED", true];
